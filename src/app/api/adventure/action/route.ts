...
export async function POST(request: NextRequest) {
  try {
    ...
    const imagePrompt = `${previousScene.substring(0, 150)}. Player action: ${command}. Pixel art style, fantasy video game scene, retro RPG aesthetic, 16-bit graphics, detailed game environment, magical atmosphere`;

    const [storyResult, imageResult] = await Promise.allSettled([
      // 1. Generate story continuation (Gemini 2.0 Flash)
    ...
    if (imageResult.status === 'fulfilled') {
      ...
      if (imageData?.image?.imageBytes) {
        ...
        try {
          imageUrl = await saveBase64Image(base64Image, 'action');
