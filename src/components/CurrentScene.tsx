...
export function CurrentScene({ currentScene: propScene, isTyping: propIsTyping, translations: propTranslations }: CurrentSceneProps) {
    ...
    const words = useMemo(() => {
        ...
        // Use a regex that keeps track of indices
        const regex = /(\s+)/;
        return currentScene.split(regex);
    }, [currentScene]);

    // Calculate start indices for each word to match with SpeechSynthesis onboundary charIndex
    const wordBoundaries = useMemo(() => {
        let currentPos = 0;
        return words.map(word => {
            const start = currentPos;
            currentPos += word.length;
            return { word, start, end: currentPos };
        });
    }, [words]);
    ...
    const handleSpeak = () => {
        ...
        // Apply "Emotion/Style" via pitch and rate
        if (voiceStyle === 'narrator') {
            utterance.rate = 0.9; // Slightly slower, more deliberate
            utterance.pitch = 0.8; // Deeper, more mysterious
        } else {
            utterance.rate = 1.1; // Faster, more heroic
            utterance.pitch = 1.2; // Brighter
        }

        // Karaoke Logic: Hook into word boundary event
    ...
    const toggleVoiceStyle = () => {
        ...
        if (isSpeaking) {
            window.speechSynthesis.cancel();
            handleSpeak(); // Restart with new style if already speaking
        }
    };
        ...
        <Card className="border-2 border-purple-400/40 bg-black/95 shadow-[0_0_30px_rgba(168,85,247,0.2)] overflow-hidden transition-all duration-500">
            ...
            <div className="bg-gradient-to-r from-purple-900/40 via-purple-600/20 to-pink-900/40 p-3 border-b-2 border-purple-400/30 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-purple-300 font-bold tracking-[0.2em] font-pixel text-[10px] md:text-sm uppercase">
                        {t.currentScene}
                    </h2>
                </div>
                    ...
                    {currentScene && !isTyping && (
                        <>
                            <Button
                                ...
                                title={voiceStyle === 'narrator' ? 'Style: Narrator' : 'Style: Hero'}
                            >
                            ...
                            <Button
                                ...
                                size="sm"
                                onClick={handleSpeak}
                                className={`h-8 gap-2 bg-purple-900/20 hover:bg-purple-800/40 text-purple-300 border border-purple-400/30 px-3 ${isSpeaking ? 'bg-pink-900/30 text-pink-400 border-pink-400/50' : ''}`}
                            >
                                {isSpeaking ? (
                                    <>
                ...
                {isTyping ? (
                    ...
                    <div className="relative z-10">
                        <div className="flex flex-wrap items-baseline gap-x-[0.3em] gap-y-1">
                            {wordBoundaries.map((item, idx) => {
                                // Find if this word should be highlighted (Karaoke)
                                const isHighlighted = isSpeaking && currentCharIndex >= item.start && currentCharIndex < item.end;
                                // Or has it already been spoken?
                                ...
                                return (
                                    <motion.span
                                        ...
                                        className={`text-base md:text-xl font-pixel transition-all duration-200 leading-relaxed
                                            ${isHighlighted
                                                ? 'text-yellow-400 scale-110 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)] z-20'
                                                : isSpoken
                                                    ? 'text-purple-300/80'
                                                    : 'text-gray-200'
                                            }
                            ...
                            <motion.span
                                ...
                                className="inline-block w-2.5 h-6 bg-purple-400 ml-1 shadow-[0_0_10px_#a855f7] align-middle"
                            />
                        </div>
                    </div>
                ) : (
                    <div className="flex flex-col items-center justify-center py-20 opacity-20">
                        ...
                        <p className="text-gray-400 font-pixel text-xs uppercase tracking-[0.4em]">{t.waitingForCommand}</p>
                    </div>
                )}

                {/* Decorative Elements */}
                <div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-[0.03] overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-[1px] bg-purple-400 animate-[scanline_10s_linear_infinite]" />
                    <pre className="font-mono text-[8px] leading-[8px] p-2">
                        {Array(20).fill("01011010111101010101").join("\n")}
                    </pre>
                </div>
            </div>
        </Card>
